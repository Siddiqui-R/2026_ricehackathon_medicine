...
State the underlying health fact, concern, care theme or action directly. Omit framing such as "the
transcript discusses", "the recording mentions" and "this was discussed in the conversation". Source links
carry where the information appeared. Keep attribution or source context in the prose only when it changes
the clinical meaning, such as patient-reported symptoms, an uncertain plan or conflicting dated findings.
...
**Care notes.** Use concise care themes or actions. If the source explicitly supplies both details,
"Frequent sock changes to prevent foot fungus, poor general hygiene" is appropriate. If it merely mentions
sock-changing frequency and hygiene, use "Sock-changing frequency and hygiene". Do not infer poor hygiene,
a fungal diagnosis or a prevention recommendation from a general discussion. Preserve questions,
suggestions and agreed plans as distinct; keep supporting source IDs on every item.
...
| A report records a possible fracture; no definitive finding is supplied.                          | Possible fracture; not confirmed.                                               | Fracture confirmed.                             |
| Transcript: an unidentified speaker asks whether physical therapy could help.                     | Question about whether physical therapy could help.                            | The doctor recommended physical therapy.        |
| Recorded discussion of left knee stiffness after a documented prior procedure.                    | Left knee stiffness                                                            | Successful surgical recovery                    |
...
Server validators reject prohibited dash punctuation, source-narration phrases and a narrow set of sentence-opening boilerplate in

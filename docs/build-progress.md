...
## Appointment recording focus — September 12, 2026

Removed appointment calling and its simulation from web, native, server routes/configuration and active setup documents. Added a dedicated full-transcript AI summary, distinct from personal notes, to the existing audio capture/transcription flow. Doctor/all-present consent gates recording; browser uploads/save require it too. Atomic summary/memory updates preserve edited notes, source timing and audio, reject stale work and invalidate derived output after transcript edits. New memory dates use Central time without overwriting user-edited dates.

Updated the current [spec comparison](reva-stack-spec.md) and [stack diagram](architecture.md). [Verification](verification/appointment-recording.md): 216 web tests, seven proxy checks, 57 core tests plus real local native/server integration, 53 server tests, state/audio/optimization harnesses, web/server/iOS builds, formatting/structure, and browser/simulator consent walkthroughs passed. Two live PostgreSQL gates were skipped; live providers and physical capture were not exercised. Previous snapshot booking data and the approved glass tab behavior are preserved.

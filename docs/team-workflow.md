...
| **Person 2 — visits, recording, visit memory** | Native `Features/Visits/**`; browser `apps/web/src/features/visits/**`, including browser visit preparation/print | Native `State/AppStore+Recordings.swift`, `+Transcription.swift`, `Device/AudioServices.swift`; browser recorder/date/edit helpers and their colocated tests. |
...
4. Appointment recording requires the doctor’s and everyone present’s consent. Keep original audio, full timestamped transcript, generated summary and personal notes distinct. Transcript edits invalidate AI output; notes edits preserve it. Calling and simulation are removed; legacy booking data remains inert.
5. Provider mocks use synthetic inputs and intercept outbound transport. Root `.env` stays empty and ignored. Account/provider/database setup and credentialed smoke tests are manual; no paid request or real call is part of this development checklist. See the contract for exact provider behavior instead of assuming model capabilities or speaker diarization.
...
- [ ] Brief simulator check: import/review → saved record → relevant brief/source/PDF; consent-gated recording and provider setup state; sample transcript correction → saved memory/backlink; relaunch. Inspect new provider controls with missing configuration and mocked success/failure as available. Device input and live credentials remain separately reported.
...
| Person 2 | Check appointment summary UI and consent-gated recording. | Consent is required before capture; AI summaries are separate from personal notes; transcription/source edits invalidate stale output. |
...
| Person 3 | Verify grounded appointment transcript summaries. | The full transcript reaches the bounded provider request; unknown details and speaker roles are not inferred; failures preserve saved source data. |

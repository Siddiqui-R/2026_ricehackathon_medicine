> **Current implementation authority — user selected September 12, 2026 (later the same day):** the app uses the heart-red palette in `design/palette.json`, chosen from rendered pink/red comparisons (option M, primary softened to `#B84250`), in **light appearance only**. Blush ivory `#FBF7F5` is the page canvas; white cards carry a linen `#DBCBC9` hairline outline at 16 pt radius; heart red `#B84250` is the action color, selected navigation and the heart glyph; deep red `#8C2F3B` is used for chip labels and small red text; petal `#FAE6E5` fills chips, notices and circular icon tiles. See the implemented [Theme.swift](../apps/ios/Reva/Features/Shared/Theme.swift). The previously reaffirmed Sky-canvas / Ivory-card / Teal-action palette is superseded and its sampled values are preserved in `design/palette-supplied.json`. Red is reserved for one primary action per screen plus status chips; body copy stays neutral; errors keep system alerts. **Every conflicting dark/adaptive, Sky-canvas, teal, or A–D proposal below is historical design history, not implementation guidance.** Local content is labeled **Local excerpt** or **Demo summary**; configured provider output uses an **AI summary** label with its model. Provider adapters now exist, but live credentials and calls remain manually unverified. See [profile and symptom-entry verification](verification/profile-symptoms.md) for the earlier palette's checks; the heart-red palette awaits its own visual check in the running MVP.
...
Heart red and ivory, outlined treatment. Blush ivory `#FBF7F5` canvas, white cards with a linen `#DBCBC9` hairline outline at 16 pt radius, heart red `#B84250` for primary actions, selected navigation and the heart glyph, deep red `#8C2F3B` for chip labels and small red text, petal `#FAE6E5` for status chips, notices and circular icon fills. Contrast: white on heart red 5.3:1, heart red on canvas 5.0:1, deep red on petal 6.8:1. Red is reserved for one action per screen plus status chips; the pale tones carry the warmth. A red cross is not used; the health cue is the heart and pulse. The supplied Ivory/Gold/Slate/Teal/Aqua/Sky swatches are preserved for provenance in [palette-supplied.json](../design/palette-supplied.json). See [design/README.md](../design/README.md). Earlier A–D options below are historical, not pending choices.

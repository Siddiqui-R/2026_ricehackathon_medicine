> **Current implementation authority — user reaffirmed September 12, 2026:** the app uses the exact supplied palette in **light appearance only**, regardless of the device's appearance setting. Ivory `#FAF4F4` is the canvas and filled-button text; Sky `#E1ECEE` is the card/supporting surface; Teal `#0A5B6C` is the action color. Gold `#C8A07D`, Slate `#A2B7BC` and Aqua `#6FABB6` retain their exact source values for restrained accents. See [palette.json](../design/palette.json) and the implemented [Theme.swift](../apps/ios/Reva/Features/Shared/Theme.swift). No adaptive dark substitutions or white-card replacement are current. **Every conflicting dark/adaptive, white-card, or A–D proposal below is historical design history, not implementation guidance.** Local content is labeled **Local excerpt** or **Demo summary**; configured provider output uses an **AI summary** label with its model. Provider adapters now exist, but live credentials and calls remain manually unverified. The current palette was visually verified in the running iPhone MVP; see the [corrected Summary screenshot](verification/screenshots/summary-exact-palette.png).

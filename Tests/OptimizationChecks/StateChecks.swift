...
@main struct StateChecks {
    ...
    @MainActor static func checkUploads(_ store: AppStore, fixture: AppSnapshot) async throws {
        var source = fixture
        for i in source.records.indices { source.records[i].sourceFilename = "shared.m4a" }
        source.recordings = [
            VisitRecording(
                visitID: source.visits[0].id, title: "Synthetic", duration: 1,
                audioFilename: "shared.m4a")
        ]
        _ = try store.repository.storeAttachment(Data([1, 2, 3]), filename: "shared.m4a")
        store.snapshot = source
        ServerClient.uploads = []
        ServerClient.pushes = 0
        await store.sync(url: store.connectionURL, token: store.connectionToken, action: "push")
        assert(ServerClient.uploads == ["shared.m4a"], "R6: shared file uploaded more than once")
        assert(ServerClient.uploadedTypes["shared.m4a"] == "audio/mp4", "R6: final metadata changed")
        ...
        print("PASS R6: one upload per file, original metadata precedence, failure/retry behavior")
    }

...
private final class ProviderTestTransport {
    ...
    init(token: String = "synthetic-provider-" + UUID().uuidString,
         handler: @escaping ProviderStubProtocol.Handler) throws {
        self.token = token
        ...
        do {
            client = try ProviderClient(url: "https://reva.example.test/gateway/", token: token, session: session)
        } catch {
...
final class ProviderClientTests: XCTestCase {
    private let transcriptJSON = #"{"text":"SYNTHETIC conversation only.","segments":[{"id":"segment-a","start":0,"end":0.625,"speaker":"Speaker","text":"SYNTHETIC opening."},{"id":"segment-b","start":5.25,"end":9.875,"speaker":"Speaker","text":"Fictional follow-up."}],"model":"synthetic-transcription-model"}"#

    private func record() -> MedicalRecord {
        MedicalRecord(id: "synthetic-record-a", title: "SYNTHETIC café note", kind: "Notes",
    ...
    private func visit() -> Visit {
        Visit(id: "synthetic-visit-a", title: "SYNTHETIC preparation", type: "Primary care",
    ...
    private func callInput() -> LiveCallInput {
        LiveCallInput(requestID: "synthetic-request-42", clinic: "Fictional clinic", phone: "+12025550123",
    ...
    private func assertProviderMessage(_ error: Error, equals expected: String,
                                       file: StaticString = #filePath, line: UInt = #line) {
    ...
    func testStatusUsesBearerAndPrefixedGETAndDecodesIndependentCapabilities() async throws {
        ...
        let transport = try ProviderTestTransport(token: token) { request in
            ...
            return ProviderStubResponse(json: #"{"gemini":{"configured":true,"model":"synthetic-summary-model"},"transcription":{"configured":false,"model":"synthetic-speech-model"},"booking":{"configured":true,"model":"synthetic-agent"},"liveCallsEnabled":false}"#)
        }
        ...
        XCTAssertEqual(result.transcription, ProviderCapability(configured: false, model: "synthetic-speech-model"))
    ...
    func testSummarySendsExactFullSourceTextAndOnlyContractFields() async throws {
        ...
        let transport = try ProviderTestTransport { request in
            ...
            return ProviderStubResponse(json: #"{"summary":"Synthetic returned summary; no change documented.","model":"synthetic-summary-model"}"#)
        }
    ...
    func testPreparationMapsEditedQuestionsCandidatesFullTextAndVersions() async throws {
        ...
        let first = record()
        var second = first
        second.id = "synthetic-record-b"; second.title = "SYNTHETIC second record"
        second.date = "2026-08-03"; second.text = "Second synthetic full source.\nExact ending."
        second.summary = "Second local excerpt."; second.version = 2
        ...
        let transport = try ProviderTestTransport { request in
            ...
            return ProviderStubResponse(json: #"{"overview":"Synthetic overview.","questions":["Synthetic returned question?"],"selectedRecordIDs":["synthetic-record-b"],"model":"synthetic-preparation-model"}"#)
        }
        ...
        XCTAssertEqual(appointment.questions, ["Keep my edited question?", "Preserve café and 0.25 exactly?"])
    }
    ...
    func testAudioPreservesRawBytesMIMEFilenameAndRelativeTimesAndRejectsSizeErrors() async throws {
        ...
        let formats = [("Fictional café.M4A", "audio/mp4", "Fictional caf_.M4A"),
        ...
        for (filename, type, metadata) in formats {
            ...
            XCTAssertEqual(result.segments, [
                TranscriptSegment(id: "segment-a", speaker: "Speaker", start: 0, end: 0.625, text: "SYNTHETIC opening."),
                TranscriptSegment(id: "segment-b", speaker: "Speaker", start: 5.25, end: 9.875, text: "Fictional follow-up.")
            ])
        }
        let calls = ProviderRequestCount()
        let transport = try ProviderTestTransport { request in
        ...
        _ = try await transport.client.transcribe(bytes: Data(repeating: 0x53, count: 16 * 1024 * 1024), filename: "synthetic-limit.m4a")
        for invalid in [Data(), Data(repeating: 0x53, count: 16 * 1024 * 1024 + 1)] {
            await expectFailure({ try await transport.client.transcribe(bytes: invalid, filename: "synthetic-invalid.m4a") }) { error in
    ...
    func testCallStartPreservesReviewedFieldsConsentAndStableRequestIdentity() async throws {
        ...
        let transport = try ProviderTestTransport { request in
            ...
            XCTAssertEqual(request.value(forHTTPHeaderField: "Content-Type"), "application/json")
            let input = try self.json(request)
            XCTAssertEqual(Set(input.keys), Set(["requestID", "clinic", "phone", "reason", "earliest", "latest", "timeZone", "preferences", "patientName", "consent"]))
            ...
            if !consent { return ProviderStubResponse(status: 400, json: #"{"reason":"Explicit consent is required."}"#) }
            return ProviderStubResponse(json: #"{"conversationID":"synthetic-conversation-42","status":"initiated","provider":"synthetic-call-provider"}"#)
        }
    ...
    func testCallStatusUsesRequestIDRouteAndRejectsUnsafePaths() async throws {
        ...
        let transport = try ProviderTestTransport(token: token) { request in
            ...
            XCTAssertEqual(request.value(forHTTPHeaderField: "Authorization"), "Bearer " + token)
            XCTAssertTrue(try self.body(request).isEmpty)
            return ProviderStubResponse(json: #"{"conversationID":"synthetic-conversation-42","status":"done","provider":"synthetic-call-provider","transcript":"SYNTHETIC call ended; no appointment confirmed."}"#)
        }
    ...
    func testServiceAndAuthFailuresSurfaceSafeMessagesWithoutBodyLeakOrRetry() async throws {
        let source = record()
        let cases = [(503, #"{"reason":"Document summarization is not configured."}"#, "Document summarization is not configured."),
                     (401, "<html>SYNTHETIC-UPSTREAM-BODY-MUST-NOT-LEAK</html>", "The connected service returned HTTP 401. Your local source is unchanged.")]
    ...
    func testMalformedSuccessfulResponsesFailInsteadOfCreatingUsableProviderResults() async throws {
        let cases = [
            ...
            ("status", #"{"gemini":{"configured":"yes","model":"synthetic"},"transcription":{"configured":false,"model":"synthetic"},"booking":{"configured":false,"model":"synthetic"},"liveCallsEnabled":false}"#),
            ("prepare", #"{"overview":"SYNTHETIC","questions":[],"selectedRecordIDs":"synthetic-record-a","model":"synthetic"}"#),
            ("audio", #"{"text":"SYNTHETIC","segments":[{"id":"segment-a","start":"tomorrow","end":1,"speaker":"Speaker","text":"SYNTHETIC"}],"model":"synthetic"}"#),
        ...
        for (kind, payload) in cases {
            ...
            await expectFailure({
                ...
                case "audio": _ = try await transport.client.transcribe(bytes: Data([0x53]), filename: "synthetic.m4a")
                default: _ = try await transport.client.startCall(self.callInput())
                }
            }) { error in
                self.assertProviderMessage(error, equals: "The connected service returned an unsupported response. Your original source is unchanged.")
            }
            XCTAssertEqual(calls.value, 1)
        }
    }

...
final class SymptomEntryTests: XCTestCase {
    ...
    private func entry() -> SymptomEntry {
        SymptomEntry(observedAt: "2026-09-12T02:15:00Z", timeZone: "America/Chicago", symptom: "Nausea",
    ...
    func testOnlySymptomAndOccurrenceAreRequiredAndUnsetSeverityRemainsAbsent() throws {
        let input = SymptomEntry(observedAt: "2026-09-12T02:15:00Z", timeZone: "America/Chicago", symptom: "  Headache  ")
    ...
    func testInvalidDatesSeverityAndEmptySymptomCannotBecomeSources() throws {
        ...
        for symptom in ["", " \n ", String(repeating: "a", count: 121), "First\nSecond"] {
            input = entry(); input.symptom = symptom
            XCTAssertThrowsError(try input.makeRecord(now: now), symptom)
        }
        for date in ["not-a-date", "2026-09-12", "2026-02-30T10:00:00Z", "2026-09-12T29:00:00Z", "2026-09-13T10:00:00Z"] {
            input = entry(); input.observedAt = date
            XCTAssertThrowsError(try input.makeRecord(now: now), date)
        }
        input = entry(); input.timeZone = "invalid/zone"
        XCTAssertThrowsError(try input.makeRecord(now: now))
        input = entry(); input.severity = "critical"
        XCTAssertThrowsError(try input.makeRecord(now: now))
        input.severity = "  MILD "
    ...
    func testEditingPreservesIdentityUploadTimeNotesAndCorrectsSource() throws {
        ...
        var imported = original; imported.symptomEntry = nil
    ...
    func testLegacySnapshotAndNewEntriesRoundtripThroughDurableRepository() throws {
        ...
        var snapshot = try JSONDecoder().decode(AppSnapshot.self, from: JSONSerialization.data(withJSONObject: legacyJSON))
    ...
    func testRelevantEntryIsQuotedWithSelfReportedProvenanceAndEditsInvalidateBrief() throws {
        ...
        var visit = Visit(title: "Discuss nausea", type: "Primary care", provider: "Example clinician", clinic: "Example clinic",
            date: "2026-09-14T16:00:00Z", concern: "Nausea after dinner", goal: "Discuss symptom timing")
        var unrelated = entry(); unrelated.symptom = "Ear pain"; unrelated.details = "Started this morning."
        unrelated.triggers = ""; unrelated.whatHelped = ""
        ...
        var correction = entry(); correction.details = "Nausea lasted an hour, not 20 minutes."

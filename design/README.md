...
| --- | --- | --- | --- |
| Ivory | 250, 244, 244 | `#FAF4F4` | Warm card surface and action text |
...
| Sky | 225, 236, 238 | `#E1ECEE` | Opaque app background and supporting accents |

These are exact pixels in the supplied image, not a claim about a separate original designer file. Preserve these named source colors unchanged. The user reaffirmed these exact values during MVP verification. The shipped MVP uses Sky canvas, Ivory cards, Teal actions and Ivory action text. Gold/Slate/Aqua remain named supporting colors. Derived dark substitutions were removed; the MVP stays in light appearance. Native system text and controls retain readable neutral rendering.

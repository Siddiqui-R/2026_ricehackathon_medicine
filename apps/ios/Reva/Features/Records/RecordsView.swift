...
struct RecordsView: View {
    ...
    let filters = ["All", "Symptoms", "Needs review", "Notes", "Labs", "Imaging", "Procedure", "Scan", "Recording"]
    var filtered: [MedicalRecord] {
        store.records.filter { record in
            (filter == "All" || (filter == "Symptoms" ? record.symptomEntry != nil : filter == "Needs review" ? record.needsReview : record.kind == filter)) &&
    ...
    var body: some View {
        Page {
            ...
            if filtered.isEmpty {
                ...
                Button(filter == "Symptoms" ? "Log symptoms" : "Add a record") { if filter == "Symptoms" { loggingSymptoms = true } else { adding = true } }.buttonStyle(PrimaryButtonStyle())
            } else {
                RevaCard {
            ...
            } label: { Image(systemName: "plus") }.accessibilityLabel("Add record or symptom entry") } }
...
struct RecordDetailView: View {
    ...
    var body: some View {
        Group {
            if let record = store.record(id) {
                Page {
                    ...
                    if record.symptomEntry == nil { DetailLine(symbol: "calendar", text: RevaDate.display(record.date)) }
                    DetailLine(symbol: record.symptomEntry == nil ? "building.2" : "person", text: record.provider)
                    ...
                    if record.symptomEntry == nil || record.summaryModel != nil { RevaCard {
                        Label(record.summaryModel != nil || hasAuthoredSummary(record) ? record.summaryLabel : "Local excerpt", systemImage: "text.alignleft").font(.headline).foregroundStyle(RevaTheme.accent)
                        ...
                        Text(record.summaryModel != nil ? "Generated by the connected AI service. Check details against the original source." : hasAuthoredSummary(record) ? "Prepared from fictional source material for this demo." : "An automatic excerpt of extracted text created on this device.").font(.caption).foregroundStyle(.secondary)
                    } }
                    if store.useConnectedAI {
                        Button(store.isProviderBusy ? "Working with connected service…" : "Summarize with Gemini") { Task { await store.summarizeWithAI(record.id) } }.buttonStyle(.bordered).disabled(store.isProviderBusy || record.text.isEmpty)
                    }
                    ...
                    if record.symptomEntry == nil { RevaCard {
                    ...
                    Button(record.symptomEntry == nil ? "Edit record & review text" : "Edit symptom entry") { editing = true }.buttonStyle(PrimaryButtonStyle())
                    Button(record.symptomEntry == nil ? "Delete record" : "Delete symptom entry", role: .destructive) { deleting = true }.frame(maxWidth: .infinity).padding(.top, 5)
                }
                .sheet(isPresented: $editing) { NavigationStack { if record.symptomEntry != nil { SymptomEntryEditorView(record: record) } else { RecordEditorView(record: record) } } }
        ...
        }.navigationTitle(store.record(id)?.symptomEntry == nil ? "Record" : "User symptom entry").navigationBarTitleDisplayMode(.inline)
...
struct RecordEditorView: View {
    ...
    private func save() {
        ...
        if textChanged {
            revised.summary = ReportEngine.localExcerpt(record.text); revised.summaryModel = nil
        ...
        Page {
            if working {
                ...
                RevaCard {
                    ...
                    Divider()
                    Button { loggingSymptoms = true } label: { intakeRow("Log symptoms", subtitle: "Record what you felt and when", symbol: "heart.text.clipboard") }
                }.buttonStyle(.plain)
            ...
            .sheet(isPresented: $sample) { NavigationStack { List {
                ...
                Button("Scanned symptom note · image-only PDF") { sample = false; if let url = Bundle.main.url(forResource: "reva-synthetic-symptom-diary-image-only", withExtension: "pdf") { Task { await ingest(url) } } }
        ...
        VStack(alignment: .leading, spacing: 18) {
            ...
            Button("Save to Records") {
                ...
                let saved = store.perform {
                    ...
                    if store.useConnectedAI && !record.text.isEmpty { Task { await store.summarizeWithAI(record.id) } }
                }
                if saved { dismiss() }
...
struct SourcePreview: View {
    ...
    var body: some View { NavigationStack {
        Group { if url.pathExtension.lowercased() == "pdf" { NativePDFView(url: url, page: page) } else { QuickLookView(url: url) } }
            .navigationTitle(title == "Visit brief" ? "Visit brief" : "Original source").navigationBarTitleDisplayMode(.inline).toolbar { ToolbarItem(placement: .cancellationAction) { Button("Done") { dismiss() } }; ToolbarItem(placement: .primaryAction) { ShareLink(item: url) { Image(systemName: "square.and.arrow.up") } } }
    } }
}

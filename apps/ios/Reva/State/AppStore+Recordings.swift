...
extension AppStore {
    ...
    func saveMemory(recordingID: String, correctedSegmentTexts: [String: String]? = nil) throws {
        try mutate { data in
            ...
            record.summary = ReportEngine.localExcerpt(sourceText); record.summaryModel = nil
            let origin = recording.isSample ? "Fictional sample transcript. No matching audio." : recording.transcriptionModel.map { "Transcript generated by " + $0 + "; review for accuracy." } ?? "Saved visit memory from user-entered notes."

...
extension AppStore {
    ...
    func runBackgroundUpdates() async {
        ...
        while !Task.isCancelled && backgroundActive && backgroundGeneration == generation {
            ...
            if Date().timeIntervalSince(lastProviderCheck) > 60
                && (account != nil || useConnectedAI || hasPendingRecordings)
            {
    ...
    func considerMedicalProfileUpdate() {
        guard backgroundActive, !providerWorkSuspended, let snapshot else { return }
    ...
    private func updateMedicalProfile(
        ...
        guard !Task.isCancelled, backgroundActive, !providerWorkSuspended, generation == backgroundGeneration,
        ...
        do {
            ...
            try Task.checkCancellation()
            guard backgroundActive, !providerWorkSuspended, generation == backgroundGeneration,
                context == providerContext,

...
enum ReportPDFError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .tooMuchContent: return "This report is too long to export. Shorten its text or source list and try again."
        case .layoutFailed: return "This report could not be formatted as a PDF. Try exporting a shorter report."
        }
    }
}
...
enum ReportPDFRenderer {
    ...
    static func render(title: String, subtitle: String, sections: [PDFSection], sources: [String]) throws -> URL {
        guard !title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else { throw ReportPDFError.emptyTitle }
        let totalCount = title.utf16.count + subtitle.utf16.count
        ...
        let textPath = CGPath(rect: CGRect(x: bodyRect.minX, y: page.height - bodyRect.maxY,
        ...
        while position < content.length {
            ...
            let frame = CTFramesetterCreateFrame(framesetter, CFRange(location: position, length: 0), textPath, nil)
            ...
            if position + length < content.length {
                ...
                let heading = content.attribute(.revaPDFHeading, at: position + length - 1, effectiveRange: &effectiveRange) as? Bool
        ...
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent("RevaExports", isDirectory: true)
        ...
        do {
            try renderer.writePDF(to: url) { context in
                for (index, range) in ranges.enumerated() {
                    context.beginPage()
                    drawPageChrome(context.cgContext, pageNumber: index + 1, pageCount: ranges.count, sourceCount: sources.count)
    ...
    private static func makeContent(title: String, subtitle: String, sections: [PDFSection], sources: [String]) -> NSAttributedString {
        ...
        for section in sections {
            if !section.title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                append(section.title, font: .systemFont(ofSize: 14, weight: .semibold), color: teal,
                       spacingBefore: 10, spacingAfter: 6, heading: true, to: text)
            }
            append(section.body.isEmpty ? "No information entered." : section.body,
                   font: .systemFont(ofSize: 11), color: .black, spacingAfter: 10, to: text)
        }
        append("Sources", font: .systemFont(ofSize: 14, weight: .semibold), color: teal,
               spacingBefore: 12, spacingAfter: 6, heading: true, to: text)
        if sources.isEmpty {
            append("No source citations were supplied for this export. Review the report before relying on it.",
                   font: .systemFont(ofSize: 10), color: .darkGray, spacingAfter: 7, to: text)
        } else {
            for (index, source) in sources.enumerated() {
                append("\(index + 1). \(source)", font: .systemFont(ofSize: 10), color: .darkGray, spacingAfter: 7, to: text)
            }
        }
        return text
    }

    private static func append(_ string: String, font: UIFont, color: UIColor, spacingBefore: CGFloat = 0,
        ...
        text.append(NSAttributedString(string: string + "\n", attributes: [
            .font: font, .foregroundColor: color, .paragraphStyle: paragraph, .revaPDFHeading: heading
        ]))
    }

    private static func drawPageChrome(_ context: CGContext, pageNumber: Int, pageCount: Int, sourceCount: Int) {
        let headerStyle: [NSAttributedString.Key: Any] = [.font: UIFont.systemFont(ofSize: 9, weight: .semibold), .foregroundColor: teal]
        ("REVA  /  VISIT PREPARATION" as NSString).draw(in: CGRect(x: 44, y: 32, width: 524, height: 16), withAttributes: headerStyle)
        ...
        let style: [NSAttributedString.Key: Any] = [.font: UIFont.systemFont(ofSize: 8), .foregroundColor: UIColor.darkGray]
        ...
        ("\(pageNumber) / \(pageCount)" as NSString).draw(in: CGRect(x: 480, y: 742, width: 88, height: 20),
            withAttributes: [.font: UIFont.systemFont(ofSize: 8), .foregroundColor: UIColor.darkGray, .paragraphStyle: right])
    }

    private static var teal: UIColor { UIColor(red: 10 / 255.0, green: 91 / 255.0, blue: 108 / 255.0, alpha: 1) }
}

...
struct ImportedDocument: Sendable {
    ...
    init(filename: String, mimeType: String, data: Data, text: String,
...
enum DocumentImportError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .tooLarge: return "This file exceeds the 16 MB import limit. Export a smaller copy and try again."
        case .unsupportedType: return "This file type is not supported. Choose a PDF, plain text file, JPEG, PNG, or HEIC image."
        case .unreadableFile: return "This file could not be read. Download it in Files first, then try again."
        case .invalidPDF: return "This PDF is damaged or contains no readable pages. Export a new PDF and try again."
        case .lockedPDF: return "This PDF is password-protected. Unlock it and export a copy before importing."
        ...
        case .invalidText: return "This text file could not be decoded. Save it as UTF-8 or UTF-16 plain text."
        }
    }
}
...
actor DocumentImportService {
    ...
    func ingest(url: URL) async throws -> ImportedDocument {
        ...
        if type?.conforms(to: .plainText) == true || ["txt", "text", "md"].contains(url.pathExtension.lowercased()) {
    ...
    private static func safeFilename(_ filename: String) -> String {
        let leaf = filename.replacingOccurrences(of: "\\", with: "/").split(separator: "/").last.map(String.init) ?? "document"
    ...
    private nonisolated func extractPlainText(data: Data, filename: String) throws -> ImportedDocument {
        ...
        let cleaned = decoded.replacingOccurrences(of: "\u{FEFF}", with: "").trimmingCharacters(in: .whitespacesAndNewlines)
        var warnings: [String] = []
        var remaining = Self.maximumTextCharacters
        let text = limitedText(cleaned, remaining: &remaining, warnings: &warnings)
        if text.isEmpty { warnings.append("No readable text was found. Review the original and enter text manually.") }
        return ImportedDocument(filename: filename, mimeType: "text/plain", data: data,
    ...
    private nonisolated func extractImage(data: Data, filename: String) throws -> ImportedDocument {
        ...
        if CGImageSourceGetCount(source) > 1 {
            warnings.append("Only the first image frame was read. The original file retains all frames; review it for additional content.")
        }
        let result = try recognize(image)
        warnings.append(contentsOf: result.warnings)
        var remaining = Self.maximumTextCharacters
        let text = limitedText(result.text, remaining: &remaining, warnings: &warnings)
        if text.isEmpty { warnings.append("No readable text was found in this image. Review the original and enter text manually or scan it again.") }
        let mimeType = CGImageSourceGetType(source).flatMap { UTType($0 as String)?.preferredMIMEType } ?? "application/octet-stream"
        return ImportedDocument(filename: filename, mimeType: mimeType, data: data,
                                text: text, warnings: warnings, pageCount: 1, pageTexts: [text])
    }
    ...
    private nonisolated func downsample(_ source: CGImageSource) throws -> CGImage {
        guard let props = CGImageSourceCopyPropertiesAtIndex(source, 0, nil) as? [CFString: Any],
              ...
              width.isFinite, height.isFinite, width > 0, height > 0 else { throw DocumentImportError.invalidImage }
        guard width * height <= Double(Self.maximumImagePixels) else { throw DocumentImportError.imageTooLarge }
    ...
    private nonisolated func extractPDF(data: Data, filename: String) throws -> ImportedDocument {
        ...
        for index in 0..<document.pageCount {
            ...
            let pageText: String = try autoreleasepool {
                guard let page = document.page(at: index) else {
                    warnings.append("Page \(index + 1) could not be opened. Review that page in the original.")
                    return ""
                }
                ...
                guard ocrCount < Self.maximumOCRPages else {
                    warnings.append("Page \(index + 1) needs OCR beyond the 10-page OCR limit. Review this page or import a smaller PDF.")
                    return embedded
                }
                ocrCount += 1
                if !warnings.contains(Self.ocrReviewWarning) { warnings.append(Self.ocrReviewWarning) }
                guard let image = rasterize(page) else {
                    warnings.append("Page \(index + 1) could not be rendered for text recognition. Review the original.")
                    return embedded
                }
                let result = try recognize(image)
                warnings.append(contentsOf: result.warnings.map { "Page \(index + 1): \($0)" })
                if result.text.isEmpty {
                    warnings.append("No readable text was found on page \(index + 1). Review the original and add any missing text.")
                }
        ...
        if text.isEmpty { warnings.append("No readable text was extracted from this PDF. The original is preserved for manual review.") }
        return ImportedDocument(filename: filename, mimeType: "application/pdf", data: data, text: text,
    ...
    private nonisolated func rasterize(_ page: PDFPage) -> CGImage? {
        ...
        context.fill(rect)
        context.concatenate(pdfPage.getDrawingTransform(.cropBox, rect: rect, rotate: 0, preserveAspectRatio: true))
    ...
    private nonisolated func recognize(_ image: CGImage) throws -> (text: String, warnings: [String]) {
        ...
        do {
            ...
            let warnings = candidates.contains(where: { $0.confidence < 0.8 })
            ...
            return ("", ["Text recognition did not complete on this device. Review the source and enter text manually."])
        }
    }

    private nonisolated func limitedText(_ text: String, remaining: inout Int, warnings: inout [String]) -> String {
        if text.count <= remaining { remaining -= text.count; return text }
    ...
    private static let ocrReviewWarning = "Text was recognized on this device with OCR and may contain mistakes or omissions. Review it against the original before using it for an appointment."
    private static let textLimitWarning = "Text extraction stopped at 200,000 characters. The complete original is preserved; review any remaining content."
}

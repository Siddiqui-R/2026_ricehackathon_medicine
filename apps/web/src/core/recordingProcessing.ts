...
export class RecordingProcessingAutomation {
  ...
  private waiting(recording: VisitRecording, state: RevaState): string | undefined {
    ...
    if (
      (!recording.segments.length || recording.status === 'processing-retranscribing') &&
      !state.providers?.transcription.configured
    )
    ...
    const jobs = recordings
      ...
      .map((item): RecordingJob => {
        ...
        if (
          ...
          item.status === 'processing-reprocess-failed' ||
        ...
        const stage = waiting
          ...
          : this.request?.id === item.id
            ? item.segments.length && item.status !== 'processing-retranscribing'
              ? 'analyzing'
              : 'transcribing'
            : 'queued';
        return {
          ...
          ...(this.paused.has(item.id) ? { canRetry: true } : {}),
          message:
            waiting ??
            (stage === 'transcribing'
  ...
  private async run(id: string, owner: string) {
    ...
    let rebuildingTranscript = false;
    this.consider();
    try {
      let recording = current();
      if (!recording || !pending.has(recording.status) || !valid()) return;
      rebuildingTranscript = recording.status === 'processing-retranscribing';
      if (!recording.segments.length || rebuildingTranscript) {
        await this.mark(
          id,
          rebuildingTranscript ? 'processing-retranscribing' : 'processing-transcribing',
          valid,
        );
      ...
      try {
        await this.mark(
          id,
          rebuildingTranscript ? 'processing-reprocess-failed' : 'processing-failed',
          valid,
        );
      } catch {
  ...
  retry = (id: string) => {
    ...
    void this.mark(
      id,
      ['processing-reprocess-failed', 'processing-retranscribing'].includes(
  ...
  reprocess = (id: string) => {
    if (!this.active || !this.host.isWorkspaceActive() || this.request?.id === id) return;
    const recording = this.host.getState().snapshot?.recordings.find((item) => item.id === id);
    if (!recording?.audioFilename || recording.isSample || pending.has(recording.status)) return;
    const owner = this.owner;
    this.errors.delete(id);
    this.paused.delete(id);
    void this.mark(
      ...
      this.errors.set(
        ...
        error instanceof Error
          ? error.message
          : 'Processing could not restart. Your saved results were kept.',
      );
      this.consider();
    });
  };

...
The owner token, provider availability and AI toggle stay in memory. Changing identity clears server revision and capabilities. The token cannot change during a connected operation. No timer calls an AI provider; account mode debounces snapshot synchronization after local saves. A 409 preserves the browser snapshot and blocks another push until a successful explicit pull; checking connection does not adopt the conflicting write revision.
...
Legacy booking arrays remain readable and are preserved unchanged; calling, simulation, and provider-call endpoints have been removed. Appointment audio is saved atomically in the active workspace after the doctor/everyone consent acknowledgement. Transcription and transcript-only summarization are explicit actions. Optional `aiSummary`, `aiSummaryModel`, and `aiSummaryGeneratedAt` values remain separate from personal notes; changed transcripts clear those fields. Tests use injected responses and never contact a paid provider.

...
import { formatDate, localExcerpt, excerptNotice } from '../../core/domain';
...
export function RecordDetail({ id }: { id: string }) {
  ...
  const currentExcerpt = localExcerpt(record.text, record.isDemo);
  const currentExcerptNotice = excerptNotice(record.text, record.isDemo);
  const authoredDemo = record.isDemo && record.summary !== currentExcerpt;
  // Legacy saved local summaries may end inside a source line; derive this preview from current source text.
  const displayedSummary = record.summaryModel || authoredDemo ? record.summary : currentExcerpt;
  ...
  return (
    <div className="stack">
      ...
      <PageHeading
        eyebrow={record.kind}
        title={record.title}
        description={`${record.provider} · ${formatDate(record.date)}`}
        actions={
      ...
      <div className="row chip-list">
        ...
        {record.isDemo && <Badge>Fictional demo source</Badge>}
        ...
        <div className="stack">
          {record.symptomEntry ? (
            ...
            <Card className="stack">
              ...
              <p className="prose">
                {displayedSummary ||
                  currentExcerptNotice ||
                  'No readable text yet. Edit this record to add a transcription, or open its original for review.'}
              </p>
              <p className="small muted">
                {record.summaryModel
                  ? `Generated by ${record.summaryModel}. Review it against the original source.`
                  : authoredDemo
                    ? 'Authored from fictional source material for this demonstration.'
                    : (currentExcerptNotice ?? 'An excerpt of source wording prepared on this device.')}
              </p>
            </Card>
          )}
          ...
          {record.notes && (
            <Card>
              ...
              <p className="prose">{record.notes}</p>
            </Card>
          )}
        </div>

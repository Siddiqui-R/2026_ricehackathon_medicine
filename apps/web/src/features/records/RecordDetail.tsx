...
export function RecordDetail({ id }: { id: string }) {
  ...
  const summarySource = linkedRecording ? (
    ...
    <SourceLink
      label={`View source: ${record.sourceFilename ? 'source document' : 'saved record text'}`}
      onOpen={() => {
  ...
  return (
    <div className="stack">
      ...
      <PageHeading
        ...
        description={`${demoLabel(record.provider, record.isDemo)} · ${recordDateLabel(record)}`}
        actions={
      ...
      <div className="detail-grid">
        <div className="stack">
          {record.symptomEntry ? (
            ...
            <Card className="stack">
              <h2>{record.summaryModel ? 'AI summary' : authoredDemo ? 'Summary' : 'Local excerpt'}</h2>
              {(
                demoDescription(currentSummary(record), record.isDemo) ||
              ...
              <p className="small muted">
                {record.summaryModel
                  ? `Generated by ${record.summaryModel}${record.summaryGeneratedAt ? ` · ${formatDate(record.summaryGeneratedAt, true)}` : ''}. Review it against the original source.`
                  : authoredDemo
                    ? 'Prepared from source material. Review it against the original source.'
          ...
          {record.symptomEntry && record.summaryModel && (
            <Card>
              ...
              <p className="small muted">
                {record.summaryModel}
                {record.summaryGeneratedAt ? ` · ${formatDate(record.summaryGeneratedAt, true)}` : ''}
              </p>
            </Card>
          )}
          <Card>
            <details ref={sourceText} tabIndex={-1}>
              ...
              <p className="prose source-text">
                {demoSourceText(record.text, record.isDemo) || 'No readable text is saved yet.'}
              </p>
            </details>
          </Card>
          {record.notes && (
        ...
        <aside className="stack">
          <Card className="stack">
            ...
            <div className="row">
              ...
              <span>
                {record.dateSource === 'added' ? 'Source date not specified' : recordDateLabel(record)}
              </span>
            </div>
            ...
            {record.sourceFilename && (
              ...
              >
                <FileText size={18} /> {demoSourcePreview(record) ? 'Open source' : 'Open original'}
              </Button>
            )}
            {linkedRecording ? (

...
describe('recording processing status', () => {
  ...
  it('offers retry for failures and service waits, while retaining the actual error message', () => {
    for (const stage of ['failed', 'waiting'] as const) {
      const html = render([
        { ...job('one', stage), message: 'The service is unavailable. Your audio is saved.' },
      ]);

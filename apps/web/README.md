...
The demo loads the same clearly fictional dataset and original documents used by the native demo. Browser changes persist in IndexedDB. Production accounts, cross-device sync, and configured providers use the same-origin Node.js API deployed with the browser on Vercel. The Swift server remains available for local development.
...
Use the repository root as the Vercel project's Root Directory. The root `vercel.json` is the only deployment configuration: it installs the root API and browser dependencies, builds `apps/web`, publishes `apps/web/dist`, and routes `/health` and `/v1/*` to `api/reva.mjs`. Do not set the project root to `apps/web` or add a nested deployment configuration; asset preparation needs the sibling native fixtures and the API bundles the existing SQL migrations. The [Vercel deployment guide](../../docs/deployment-vercel.md) covers build settings, Tiger PostgreSQL, authentication, and configured providers. The local Node/Vite proxy remains a separate development tool.
...
- Recording opens as a full page after a full-screen participant-consent check. The shared session keeps capturing while users navigate within the workspace; a sticky banner offers pause, resume, stop, save and return controls. Switching browser tabs pauses capture. Reload and account-exit guards protect unsaved audio, but unsaved drafts are not restored after closing the browser. Microphone capture requires a secure context (localhost/HTTPS), browser support and explicit microphone permission, and is bounded to 30 minutes or 16 MiB. Upload audio is an alternate mode in the same page; originals retain their actual container type.

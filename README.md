...
The launcher reads optional dotenv values without shell evaluation; exported environment variables take precedence. The root `.env` is local and Git-ignored; a fresh clone can create it with `touch .env`. Use the commented [root example](.env.example) and [server example](server/.env.example) for manual setup; provider secrets stay on the server.
...
The [audit repair checklist](docs/audit-repair-checklist.md) tracks the latest 22 applicable fixes, one WIP-only finding, 106 passing browser tests, focused native regressions and remaining platform/service gates. The prior Xcode results below are historical; this Windows repair checkpoint does not re-establish an iOS build or hardware verification.

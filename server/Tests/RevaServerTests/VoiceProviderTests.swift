// Purpose: Verify audio wire fidelity and durable call replay behavior without contacting a speech/calling provider.
// Inputs: Synthetic bytes/transcripts, fake configuration, controlled transport outcomes, and temporary receipt files.
// Outputs: Assertions on timestamps, consent, ownership, replay/concurrency, restart recovery, limits, and route keys.
// Side effects: Creates temporary receipts/locks and VaporTesting apps. Mock transports cannot place a real call.
...
private let voiceEnvironment = [
    "OPENAI_API_KEY": "mock-openai-key-never-used-live",
    "ELEVENLABS_API_KEY": "mock-elevenlabs-key-never-used-live",
    "ELEVENLABS_AGENT_ID": "agent_mock", "ELEVENLABS_PHONE_NUMBER_ID": "phone_mock",
...
private let outboundJSON =
    #"{"success":true,"message":"initiated","conversation_id":"conv_mock","callSid":"mock_sid"}"#
private let conversationJSON =
    #"{"conversation_id":"conv_mock","status":"done","transcript":[{"role":"agent","time_in_call_secs":0,"message":"I am calling about availability."},{"role":"user","time_in_call_secs":2.4,"message":"Please ask the patient to call us."},{"role":"agent","time_in_call_secs":3,"message":null}]}"#
...
// MARK: - Temporary receipt directories and consent-bearing request builders
...
private func callRequest(
    id: String = "request-a", phone: String = "+13125550123", consent: Bool = true,
    reason: String = "Ask about a fictional appointment", earliest: String = "2030-09-12T14:00:00Z",
    latest: String = "2030-09-20T20:00:00Z", zone: String = "America/Chicago"
) -> VoiceBookingCallRequest {
    VoiceBookingCallRequest(
        requestID: id, clinic: "Fictional Clinic", phone: phone, reason: reason,
...
struct VoiceProviderTests {
    ...
    // MARK: - Durable call intent and exact replay semantics
    @Test func callPersistsIntentBeforeTransportAndReplaysWithoutRedial() async throws {
        ...
        let mock = VoiceMockHTTP(
            [.response(200, outboundJSON), .response(200, conversationJSON)],
        ...
        let service = VoiceCallService(
            configuration: try voiceConfiguration(), directory: directory, transport: mock.transport)
        // Initialization has no filesystem effect.
        #expect(!FileManager.default.fileExists(atPath: directory.path))
        let started = try await service.start(owner: "owner-a", call: callRequest())
        #expect(
            started
                == VoiceBookingCallResponse(
                    conversationID: "conv_mock", status: "initiated", provider: "ElevenLabs"))
        ...
        #expect(
            variables["earliest"] == "2030-09-12T14:00:00Z" && variables["time_zone"] == "America/Chicago")
        try await expectVoiceAbort(.notFound) {
            _ = try await service.details(owner: "owner-b", requestID: "request-a")
        }
        try await expectVoiceAbort(.conflict) {
            _ = try await service.start(owner: "owner-a", call: callRequest(reason: "Changed request"))
        }
        #expect(await mock.count == 1)
        let details = try await service.details(owner: "owner-a", requestID: "request-a")
        // Completion is a conversation status, never appointment confirmation.
        #expect(details.status == "done")
    ...
    // MARK: - Consent and configuration gates before any dial attempt
    @Test func callGatesAndInvalidRequestsNeverDial() async throws {
        let directory = temporaryVoiceDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let mock = VoiceMockHTTP([])
        var disabled = voiceEnvironment
        disabled["REVA_ENABLE_LIVE_CALLS"] = "false"
        for configuration in [
            try voiceConfiguration([:]), try voiceConfiguration(disabled),
            try voiceConfiguration(allowed: false),
        ] {
            let service = VoiceCallService(
                configuration: configuration, directory: directory, transport: mock.transport)
            try await expectVoiceAbort(.serviceUnavailable) {
                _ = try await service.start(owner: "owner-a", call: callRequest())
            }
        }
        let service = VoiceCallService(
            configuration: try voiceConfiguration(), directory: directory, transport: mock.transport)
        for call in [
            callRequest(consent: false), callRequest(phone: "(312) 555-0123"),
            callRequest(phone: "+13125550123\n"),
            ...
            try await expectVoiceAbort(.badRequest) {
                _ = try await service.start(owner: "owner-a", call: call)
            }
        }
        #expect(await mock.count == 0)
        #expect(!FileManager.default.fileExists(atPath: directory.path))
    ...
    // MARK: - Restart, concurrency, corrupted storage, and uncertain outcomes
    @Test func uncertainCallSurvivesRestartAndNeverRedials() async throws {
        let directory = temporaryVoiceDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let mock = VoiceMockHTTP([.failure])
        var first: VoiceCallService? = VoiceCallService(
            configuration: try voiceConfiguration(), directory: directory, transport: mock.transport)
        try await expectVoiceAbort(.badGateway) {
            _ = try await first!.start(owner: "owner-a", call: callRequest())
        }
        first = nil
        let restarted = VoiceCallService(
            configuration: try voiceConfiguration(), directory: directory, transport: mock.transport)
        try await expectVoiceAbort(.conflict) {
            _ = try await restarted.start(owner: "owner-a", call: callRequest())
        }
    ...
    @Test func submittedReceiptSurvivesRestartAndDirectoryLockExcludesSecondInstance() async throws {
        ...
        var first: VoiceCallService? = VoiceCallService(
            configuration: try voiceConfiguration(), directory: directory, transport: mock.transport)
        _ = try await first!.start(owner: "owner-a", call: callRequest())
        let another = VoiceCallService(
            configuration: try voiceConfiguration(), directory: directory, transport: mock.transport)
        try await expectVoiceAbort(.serviceUnavailable) {
            _ = try await another.start(owner: "owner-a", call: callRequest())
        }
        first = nil
    ...
    @Test func corruptReceiptAndUnavailableStorageFailClosed() async throws {
        ...
        try Data("broken receipt".utf8).write(
            to: directory.appendingPathComponent("voice-call-receipts/owner-a/request-a.json"))
        try await expectVoiceAbort(.serviceUnavailable) {
            _ = try await service.start(owner: "owner-a", call: callRequest())
        }
        #expect(await mock.count == 1)
        ...
        let blocked = VoiceCallService(
            configuration: try voiceConfiguration(), directory: unavailable, transport: mock.transport)
        try await expectVoiceAbort(.serviceUnavailable) {
            _ = try await blocked.start(owner: "owner-a", call: callRequest())
        }
        #expect(await mock.count == 1)
    }
    ...
    @Test func providerFailureAndMalformedSuccessRemainUncertain() async throws {
        for outcome in [
            VoiceMockHTTP.Outcome.response(422, #"{"error":"private-provider-data"}"#),
            .response(200, #"{"success":true}"#), .response(200, "bad JSON"),
        ] {
            let directory = temporaryVoiceDirectory()
            defer { try? FileManager.default.removeItem(at: directory) }
            let mock = VoiceMockHTTP([outcome])
            ...
            do {
                _ = try await service.start(owner: "owner-a", call: callRequest())
                Issue.record("Expected uncertain provider outcome")
            } catch let error as Abort {
                #expect(error.status == .badGateway)
                #expect(!error.reason.contains("private-provider-data"))
            }
            try await expectVoiceAbort(.conflict) {
                _ = try await service.start(owner: "owner-a", call: callRequest())
            }
    ...
    // MARK: - Owner-scoped bounded polling after new calls are disabled
    @Test func pollingIsOwnerScopedBoundedAndAllowedAfterDisablingNewCalls() async throws {
        ...
        let payload: [String: Any] = [
            "conversation_id": "conv_mock", "status": "done",
            "transcript": [["role": "user", "time_in_call_secs": 1, "message": longMessage]],
        ]
        ...
        var initial: VoiceCallService? = VoiceCallService(
            configuration: try voiceConfiguration(), directory: directory, transport: mock.transport)
        _ = try await initial!.start(owner: "owner-a", call: callRequest())
        initial = nil
        var disabled = voiceEnvironment
        disabled["REVA_ENABLE_LIVE_CALLS"] = "false"
        let polling = VoiceCallService(
            configuration: try voiceConfiguration(disabled), directory: directory, transport: mock.transport)
        ...
        try await expectVoiceAbort(.notFound) {
            _ = try await polling.details(owner: "owner-b", requestID: "request-a")
        }
        try await expectVoiceAbort(.serviceUnavailable) {
            _ = try await polling.start(owner: "owner-a", call: callRequest(id: "new-request"))
        }
        #expect(await mock.count == 2)
    }

    // MARK: - HTTP authentication and stable native wire keys
    @Test func routesEnforceAuthenticationAndExactWireKeys() async throws {
        let directory = temporaryVoiceDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let mock = VoiceMockHTTP([
            .response(200, whisperJSON), .response(200, outboundJSON), .response(200, conversationJSON),
        ])
        ...
        try await withApp(configure: { app in
            ...
            for route in ["v1/audio/transcribe", "v1/booking/call"] {
                try await app.testing().test(.POST, route) { response async in
                    #expect(response.status == .unauthorized)
                }
            }
            let audioHeaders: HTTPHeaders = [
                "Authorization": "Bearer owner-a", "Content-Type": "audio/mp4", "X-Filename": "synthetic.m4a",
            ]
            try await app.testing().test(
                .POST, "v1/audio/transcribe", headers: audioHeaders, body: ByteBuffer(bytes: [0, 1, 2])
            ...
            try await app.testing().test(
                .POST, "v1/booking/call", headers: ["Authorization": "Bearer owner-a"],
            ...
            try await app.testing().test(
                .GET, "v1/booking/call/request-a", headers: ["Authorization": "Bearer owner-b"]
            ) { response async in
                #expect(response.status == .notFound)
            }
            try await app.testing().test(
                .GET, "v1/booking/call/request-a", headers: ["Authorization": "Bearer owner-a"]
            ) { response async throws in
                #expect(response.status == .ok)
                let json = try #require(
                    JSONSerialization.jsonObject(with: Data(response.body.readableBytesView))

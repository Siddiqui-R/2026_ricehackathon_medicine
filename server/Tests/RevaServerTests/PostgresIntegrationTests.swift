...
struct PostgresIntegrationTests {
    ...
    func realDatabaseRoundtripAndMigrations() async throws {
        ...
        var environment = ["REVA_STORAGE": "postgres", "DATABASE_URL": url, "REVA_TOKENS": "{\"integration-test-token-123456789\":\"test-owner\"}"]
        ...
        do {
            try await withDatabaseDeadline {
                ...
                let snapshot: JSONValue = .object(["schemaVersion": .integer(1), "profile": .object(["name": .string("Synthetic integration patient")]), "records": .array([]), "visits": .array([]), "bookings": .array([]), "recordings": .array([])])
                ...
                await #expect(throws: StoreError.self) { try await store.putState(owner: owner, baseRevision: 0, snapshot: snapshot) }
                await #expect(throws: StoreError.self) { try await store.getState(owner: owner + "x") }
                let attachment = StoredAttachment(id: "source", filename: "sample.pdf", contentType: "application/pdf", data: Data([0, 255, 128]), updatedAt: Date(timeIntervalSince1970: 10))
                ...
                await #expect(throws: StoreError.self) { try await store.getAttachment(owner: owner + "x", id: "source") }
                #expect(try await store.deleteState(owner: owner) == 2)
                await #expect(throws: StoreError.self) { try await store.getAttachment(owner: owner, id: "source") }
                #expect(try await store.deleteState(owner: owner) == 2)
                // Clean only this random synthetic owner, including its tombstone and audit.
                let rows = try await store.client.query("DELETE FROM reva_owner_state WHERE owner_id = \(owner)")

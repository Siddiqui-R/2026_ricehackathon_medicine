...
private let sample: JSONValue = .object([
    "schemaVersion": .integer(1), "profile": .object(["id": .string("owner-b"), "name": .string("Synthetic patient")]),
...
private func withServer(_ test: (Application, LocalFileStore, URL) async throws -> Void) async throws {
    let directory = FileManager.default.temporaryDirectory.appendingPathComponent("reva-tests-" + UUID().uuidString)
    defer { try? FileManager.default.removeItem(at: directory) }
    let configuration = try ServerConfiguration(environment: ["REVA_TOKENS": "{\"\(tokenA)\":\"owner-a\",\"\(tokenB)\":\"owner-b\"}"])
    let store = try LocalFileStore(directory: directory)
    try await withApp(configure: { app in configure(app, configuration: configuration, store: store) }) { app in
...
struct ServerTests {
    @Test func authOwnershipAndRevisionConflicts() async throws {
        try await withServer { app, _, _ async throws in
            ...
            for route in ["v1/state", "v1/attachments/source-1"] {
                try await app.testing().test(.GET, route) { response async in #expect(response.status == .unauthorized) }
            }
            try await app.testing().test(.GET, "v1/state", headers: headers("invalid")) { response async in
                #expect(response.status == .unauthorized)
            }
            try await app.testing().test(.PUT, "v1/state", headers: headers(), beforeRequest: { request in
            ...
            try await app.testing().test(.DELETE, "v1/state", headers: headers(tokenB)) { response async in #expect(response.status == .ok) }
            try await app.testing().test(.GET, "v1/state", headers: headers()) { response async in #expect(response.status == .ok) }
        }
    }
    ...
    @Test func attachmentRoundtripAndDeletion() async throws {
        try await withServer { app, _, _ async throws in
            ...
            try await app.testing().test(.PUT, "v1/attachments/source-1", headers: uploadHeaders, body: ByteBuffer(data: bytes)) { response async in
                #expect(response.status == .noContent)
            }
            try await app.testing().test(.GET, "v1/attachments/source-1", headers: headers()) { response async in
                #expect(response.status == .ok)
            ...
            try await app.testing().test(.GET, "v1/attachments/source-1", headers: headers(tokenB)) { response async in #expect(response.status == .notFound) }
            try await app.testing().test(.DELETE, "v1/attachments/source-1", headers: headers(tokenB)) { response async in #expect(response.status == .noContent) }
            try await app.testing().test(.GET, "v1/attachments/source-1", headers: headers()) { response async in #expect(response.status == .ok) }
            try await app.testing().test(.DELETE, "v1/attachments/source-1", headers: headers()) { response async in #expect(response.status == .noContent) }
            try await app.testing().test(.GET, "v1/attachments/source-1", headers: headers()) { response async in #expect(response.status == .notFound) }
        }
    }
    ...
    @Test func invalidPayloadsAndPathsAreRejected() async throws {
        try await withServer { app, _, _ async throws in
            try await app.testing().test(.PUT, "v1/state", headers: headers(), beforeRequest: { request in
                try request.content.encode(StateWrite(baseRevision: 0, snapshot: .object(["profile": .string("wrong")])) )
            }, afterResponse: { response async in #expect(response.status == .badRequest) })
            try await app.testing().test(.PUT, "v1/state", headers: headers(), body: ByteBuffer(string: "garbage")) { response async in
                #expect(response.status == .unsupportedMediaType)
            }
            let oversize: JSONValue = .object(["schemaVersion": .integer(1), "profile": .object(["name": .string(String(repeating: "x", count: Validation.maxSnapshotBytes))]), "records": .array([]), "visits": .array([]), "bookings": .array([]), "recordings": .array([])])
            try await app.testing().test(.PUT, "v1/state", headers: headers(), beforeRequest: { request in
            ...
            for (id, filename, type, expected) in [
                ...
                try await app.testing().test(.PUT, "v1/attachments/\(id)", headers: requestHeaders, body: ByteBuffer(string: "data")) { response async in
                    #expect(response.status == expected)
                }
            }
            try await app.testing().test(.GET, "v1/state", headers: headers()) { response async in #expect(response.status == .notFound) }
        }
    }
    ...
    @Test func deletionTombstonePreventsResurrection() async throws {
        try await withServer { app, store, _ async throws in
            ...
            try await store.putAttachment(owner: "owner-a", attachment: .init(id: "source", filename: "source.txt", contentType: "text/plain", data: Data("fictional".utf8), updatedAt: Date()))
            ...
            try await app.testing().test(.GET, "v1/attachments/source", headers: headers()) { response async in #expect(response.status == .notFound) }
            #expect(try await store.deleteState(owner: "owner-a") == 2)
            await #expect(throws: StoreError.self) { try await store.putState(owner: "owner-a", baseRevision: 1, snapshot: sample) }
    ...
    @Test func persistenceSurvivesNewStoreAndCorruptionFails() async throws {
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent("reva-restart-" + UUID().uuidString)
        defer { try? FileManager.default.removeItem(at: directory) }
        let attachment = StoredAttachment(id: "audio", filename: "synthetic.m4a", contentType: "audio/mp4", data: Data([0, 255, 128]), updatedAt: Date(timeIntervalSince1970: 10))
        ...
        await #expect(throws: StoreError.self) { try await restarted.putState(owner: "owner-a", baseRevision: 0, snapshot: sample) }
        #expect(try String(contentsOf: directory.appendingPathComponent("owner-a.json"), encoding: .utf8) == "broken persisted JSON")
    }
    ...
    @Test func simultaneousWritesHaveExactlyOneWinner() async throws {
        try await withServer { _, store, _ async throws in
            let wins = await withTaskGroup(of: Bool.self) { group in
                for _ in 0..<8 {
                    group.addTask { (try? await store.putState(owner: "owner-a", baseRevision: 0, snapshot: sample)) != nil }
                }
                var wins = 0
    ...
    @Test func explicitConfigurationFailsClosed() throws {
        ...
        #expect(throws: ConfigurationError.self) { try ServerConfiguration(environment: [:], arguments: ["serve", "--hostname", "0.0.0.0"]) }
        for environment in [
        ...
        #expect(throws: ConfigurationError.self) {
            try ServerConfiguration(environment: ["REVA_STORAGE": "postgres", "REVA_TOKENS": tokens, "DATABASE_URL": "postgres://user:secret@db.example/reva?sslmode=disable"])
        }
        let configured = try ServerConfiguration(environment: ["REVA_STORAGE": "postgres", "REVA_TOKENS": tokens, "DATABASE_URL": "postgres://user:secret@db.example/reva?sslmode=verify-full"])
    ...
    @Test func attachmentSizeAndOwnerQuotaAreEnforced() async throws {
        try await withServer { app, store, _ async throws in
            ...
            try await app.testing().test(.PUT, "v1/attachments/too-big", headers: uploadHeaders,
                                         body: ByteBuffer(repeating: 0, count: Validation.maxAttachmentBytes + 1)) { response async in
                #expect(response.status == .payloadTooLarge)
            }
            try await app.testing().test(.PUT, "v1/attachments/empty", headers: uploadHeaders) { response async in
                #expect(response.status == .badRequest)
            }
            for index in 0..<Validation.maxAttachmentCount {
                try await store.putAttachment(owner: "owner-a", attachment: .init(id: "file-\(index)", filename: "Synthetic.txt", contentType: "text/plain", data: Data([1]), updatedAt: Date()))
            }
            await #expect(throws: StoreError.self) {
                try await store.putAttachment(owner: "owner-a", attachment: .init(id: "over-quota", filename: "Synthetic.txt", contentType: "text/plain", data: Data([1]), updatedAt: Date()))
            }
            // Replacing an existing attachment does not consume another slot.
            try await store.putAttachment(owner: "owner-a", attachment: .init(id: "file-0", filename: "Synthetic.txt", contentType: "text/plain", data: Data([2]), updatedAt: Date()))

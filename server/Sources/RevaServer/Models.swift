...
struct StateWrite: Content { let baseRevision: Int; let snapshot: JSONValue }
struct RevisionResponse: Content { let revision: Int }
struct HealthResponse: Content { let status: String; let storage: String; let isDemo: Bool }
struct ErrorResponse: Content { let error: Bool; let reason: String }
...
enum StoreError: Error, Sendable {
    case conflict(Int), missing(Int), attachmentMissing, quota, corrupt
}
...
enum Validation {
    ...
    static let contentTypes: Set<String> = ["application/pdf", "text/plain", "image/png", "image/jpeg", "image/heic", "image/heif", "audio/mp4", "audio/x-m4a", "audio/m4a", "audio/mpeg", "audio/wav", "audio/x-wav", "application/octet-stream"]
    ...
    static func snapshot(_ value: JSONValue) throws {
        guard case .object(let object) = value,
              ...
              case .object(let profile) = object["profile"], !profile.isEmpty else {
            throw Abort(.badRequest, reason: "Snapshot must have schemaVersion 1 and a nonempty profile object.")
        }
        for key in ["records", "visits", "bookings", "recordings"] {
            guard case .array(let items) = object[key], items.count <= 5000,
                  items.allSatisfy({ if case .object = $0 { return true }; return false }) else {
                throw Abort(.badRequest, reason: "Snapshot requires records, visits, bookings, and recordings arrays of objects (maximum 5000 each).")
            }
        }
    ...
    static func attachment(_ attachment: StoredAttachment) throws {
        guard safeID(attachment.id) else { throw Abort(.badRequest, reason: "Attachment ID must contain only ASCII letters, numbers, hyphens or underscores (1–80 characters).") }
        let name = attachment.filename
        guard !name.isEmpty, name.utf8.count <= 180, name != ".", name != "..", !name.hasPrefix("."),
              name == name.trimmingCharacters(in: .whitespaces), name.utf8.allSatisfy({
                  (65...90).contains($0) || (97...122).contains($0) || (48...57).contains($0) || [32, 40, 41, 45, 46, 95].contains($0)
              }) else { throw Abort(.badRequest, reason: "X-Filename must be a flat ASCII filename using letters, numbers, spaces, dots, hyphens, underscores or parentheses (1–180 characters).") }
        guard contentTypes.contains(attachment.contentType) else { throw Abort(.unsupportedMediaType, reason: "Unsupported attachment Content-Type.") }
        guard !attachment.data.isEmpty else { throw Abort(.badRequest, reason: "Attachment body is empty.") }
        guard attachment.data.count <= maxAttachmentBytes else { throw Abort(.payloadTooLarge, reason: "Attachment exceeds 16 MiB.") }
    }
}

...
public actor LocalFileStore: RevaStore {
    ...
    public func putAttachment(owner: String, attachment: StoredAttachment) async throws {
        ...
        var document = try read(owner)
        let total = document.attachments.values.reduce(0) { $0 + $1.data.count } - (document.attachments[attachment.id]?.data.count ?? 0) + attachment.data.count
        guard total <= Validation.maxOwnerAttachmentBytes,
              document.attachments[attachment.id] != nil || document.attachments.count < Validation.maxAttachmentCount else { throw StoreError.quota }
...
private final class DirectoryLock: @unchecked Sendable {
    ...
    init(directory: URL) throws {
        ...
        guard flock(descriptor, LOCK_EX | LOCK_NB) == 0 else {
            close(descriptor)
            throw ConfigurationError("Local data directory is already in use by another store. Use one server process per directory.")
        }

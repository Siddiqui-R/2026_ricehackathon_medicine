...
struct GeminiService: Sendable {
    ...
    func summarize(_ request: GeminiSummaryRequest) async throws -> GeminiSummaryResponse {
        ...
        let schema: JSONValue = .object([
            "type": .string("object"), "properties": .object(["summary": .object(["type": .string("string")])]),
            "required": .array([.string("summary")]), "additionalProperties": .bool(false)
        ])
        let object = try await generate(input: request, schema: schema, task: """
        ...
        guard let summary = object["summary"] as? String, GeminiValidation.text(summary, maximum: 8000),
              Set(object.keys) == ["summary"] else { throw invalidResponse() }
    ...
    func prepare(_ request: GeminiPreparationRequest) async throws -> GeminiPreparationResponse {
        ...
        let schema: JSONValue = .object([
            ...
            "properties": .object([
                ...
                "questions": .object(["type": .string("array"), "items": .object(["type": .string("string")]), "maxItems": .integer(20)]),
                "selectedRecordIDs": .object(["type": .string("array"), "items": .object(["type": .string("string"), "enum": .array(candidateIDs.map(JSONValue.string))]), "maxItems": .integer(100)])
            ]),
        ...
        guard let overview = object["overview"] as? String, GeminiValidation.text(overview, maximum: 12_000),
              ...
              Set(object.keys) == ["overview", "questions", "selectedRecordIDs"] else { throw invalidResponse() }
        return GeminiPreparationResponse(overview: overview, questions: questions, selectedRecordIDs: selected, model: configuration.geminiModel)
    }

    private func generate<Input: Encodable>(input: Input, schema: JSONValue, task: String) async throws -> [String: Any] {
        guard configuration.paidAccessAllowed else {
            throw Abort(.serviceUnavailable, reason: "Provider access requires a private REVA_TOKENS mapping; the public local demo token cannot activate paid providers.")
        }
        guard let key = configuration.geminiAPIKey else {
            throw Abort(.serviceUnavailable, reason: "Gemini is not configured. Set GEMINI_API_KEY on the server and restart.")
        }
        ...
        let payload: JSONValue = .object([
            ...
            "contents": .array([.object(["role": .string("user"), "parts": .array([.object(["text": .string(userJSON)])])])]),
        ...
        guard let url = URL(string: "https://generativelanguage.googleapis.com/v1beta/models/\(configuration.geminiModel):generateContent") else {
        ...
        catch { throw Abort(.serviceUnavailable, reason: "Gemini could not be reached. Your local data is unchanged; retry when the connection is available.") }
        guard (200..<300).contains(response.status) else {
            throw Abort(.serviceUnavailable, reason: "Gemini rejected the request. Check server model/key access and provider quota; no provider error details are exposed.")
        }
        guard response.data.count <= 1_048_576,
              ...
              let parts = candidate.content?.parts else { throw invalidResponse() }
        let text = parts.filter { $0.thought != true }.compactMap(\.text).joined()
        guard !text.isEmpty, text.utf8.count <= 64_000,
              let object = try? JSONSerialization.jsonObject(with: Data(text.utf8)) as? [String: Any] else { throw invalidResponse() }
        return object
    }

    private func invalidResponse() -> Abort {
        Abort(.serviceUnavailable, reason: "Gemini returned incomplete, blocked or invalid structured output. No AI result was saved; retry or use local preparation.")
    }
}

...
struct VoiceTranscriptionService: Sendable {
    ...
    func transcribe(audio: Data, filename: String, contentType: String) async throws -> VoiceTranscriptionResponse {
        guard configuration.paidAccessAllowed, let key = configuration.openAIAPIKey, !key.isEmpty,
              configuration.transcriptionModel == "whisper-1" else {
            throw Abort(.serviceUnavailable, reason: "Transcription is not configured. Set private server tokens, OPENAI_API_KEY, and OPENAI_TRANSCRIPTION_MODEL=whisper-1.")
        }
        ...
        catch { throw Abort(.badGateway, reason: "The transcription provider could not be reached. Your saved audio remains available.") }
        let data = try VoiceProviderLimits.responseData(response)
        let decoded: WhisperResponse
        do { decoded = try JSONDecoder().decode(WhisperResponse.self, from: data) }
        catch { throw Abort(.badGateway, reason: "The transcription provider returned invalid transcript data.") }
        let text = decoded.text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else { throw Abort(.unprocessableEntity, reason: "No speech was transcribed. Review the saved audio before trying again.") }
        guard text.count <= VoiceProviderLimits.transcriptCharacters,
            ...
            throw Abort(.badGateway, reason: "The transcription provider did not return usable bounded segment timestamps.")
        }
        ...
        for segment in segments {
            ...
            guard segment.id >= 0, segment.start.isFinite, segment.end.isFinite,
                  segment.start >= previousStart, segment.end >= segment.start, segment.end <= decoded.duration + 1,
                  !words.isEmpty, totalCharacters <= VoiceProviderLimits.transcriptCharacters else {
                throw Abort(.badGateway, reason: "The transcription provider returned invalid segment text or timestamps.")
            }
            previousStart = segment.start
            result.append(VoiceTranscriptSegment(id: "whisper-segment-\(segment.id)", start: segment.start, end: segment.end,

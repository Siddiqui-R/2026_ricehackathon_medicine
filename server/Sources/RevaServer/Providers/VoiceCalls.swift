...
struct VoiceBookingCallRequest: Content, Sendable, Equatable {
    ...
    func validate() throws {
        guard consent else { throw Abort(.badRequest, reason: "Explicit consent is required before placing a real call.") }
        guard Validation.safeID(requestID) else { throw Abort(.badRequest, reason: "requestID must be a stable safe identifier (1–80 ASCII letters, numbers, hyphens or underscores).") }
        let digits = Array(phone.utf8)
        guard (3...16).contains(digits.count), digits.first == 43, (49...57).contains(digits[1]),
              digits.dropFirst(2).allSatisfy({ (48...57).contains($0) }) else {
            throw Abort(.badRequest, reason: "Use an exact E.164 phone number, such as +13125550123, without spaces or extensions.")
        }
        for (value, limit) in [(clinic, 200), (reason, 2_000), (patientName, 200)] {
            guard !value.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty, value.count <= limit else {
                throw Abort(.badRequest, reason: "Provide a clinic, patient name, and concise appointment reason.")
            }
        }
        guard preferences.count <= 2_000, timeZone.count <= 100, TimeZone(identifier: timeZone) != nil,
              earliest.count <= 50, latest.count <= 50,
              let start = Self.date(earliest), let end = Self.date(latest), start <= end else {
            throw Abort(.badRequest, reason: "Provide valid ISO 8601 appointment bounds, a valid time zone, and concise preferences.")
        }
    }
...
actor VoiceCallService {
    ...
    private static let providerStatuses: Set<String> = ["initiated", "in-progress", "processing", "done", "failed"]
    ...
    func start(owner: String, call: VoiceBookingCallRequest) async throws -> VoiceBookingCallResponse {
        guard configuration.bookingConfigured, configuration.liveCallsEnabled,
              ...
              let number = configuration.elevenLabsPhoneNumberID else {
            throw Abort(.serviceUnavailable, reason: "Live calling is disabled. Configure private server tokens, ElevenLabs key/agent/phone-number IDs, and REVA_ENABLE_LIVE_CALLS=true.")
        }
        try call.validate()
        try validateOwner(owner)
        if let existing = try read(owner: owner, requestID: call.requestID) {
            guard existing.request == call else { throw Abort(.conflict, reason: "This requestID already belongs to a different call request. It will not dial again.") }
            guard let id = existing.conversationID else { throw uncertainReplay() }
            return VoiceBookingCallResponse(conversationID: id, status: existing.status, provider: Self.provider)
        }
        try Task.checkCancellation()
        var receipt = VoiceCallReceipt(owner: owner, request: call, status: "intent", conversationID: nil, createdAt: Date())
        ...
        let payload = ElevenOutboundPayload(agent_id: agent, agent_phone_number_id: number, to_number: call.phone,
        ...
        var request = URLRequest(url: URL(string: "https://api.elevenlabs.io/v1/convai/twilio/outbound-call")!)
        ...
        do {
            ...
            try write(receipt)
            return VoiceBookingCallResponse(conversationID: receipt.conversationID!, status: receipt.status, provider: Self.provider)
        } catch {
            ...
            throw Abort(.badGateway, reason: "The call outcome could not be confirmed. This request will not dial again. Check ElevenLabs before authorizing any new call.")
        }
    }

    func details(owner: String, requestID: String) async throws -> VoiceBookingCallDetails {
        try validateOwner(owner)
        guard Validation.safeID(requestID) else { throw Abort(.badRequest, reason: "Invalid call request ID.") }
        guard var receipt = try read(owner: owner, requestID: requestID) else { throw Abort(.notFound, reason: "No call request exists for this owner.") }
        guard let id = receipt.conversationID else {
            return VoiceBookingCallDetails(conversationID: "", status: "uncertain", provider: Self.provider, transcript: "")
        }
        ...
        guard configuration.paidAccessAllowed, let key = configuration.elevenLabsAPIKey else {
            throw Abort(.serviceUnavailable, reason: "Configure the private ElevenLabs server key to retrieve this call.")
        }
        ...
        catch { throw Abort(.badGateway, reason: "Call status could not be retrieved. The original call request was preserved; polling will not place another call.") }
        let data = try VoiceProviderLimits.responseData(response)
        let details: ElevenConversationResponse
        do { details = try JSONDecoder().decode(ElevenConversationResponse.self, from: data) }
        catch { throw Abort(.badGateway, reason: "ElevenLabs returned invalid conversation data.") }
        guard details.conversation_id == id, Self.providerStatuses.contains(details.status), details.transcript.count <= 10_000 else {
            throw Abort(.badGateway, reason: "ElevenLabs returned an unexpected conversation or status.")
        }
        var transcript = ""
        for turn in details.transcript {
            guard let message = turn.message, !message.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else { continue }
            guard turn.time_in_call_secs.isFinite, turn.time_in_call_secs >= 0, turn.time_in_call_secs <= 24 * 60 * 60 else {
        ...
        try write(receipt)
        return VoiceBookingCallDetails(conversationID: id, status: details.status, provider: Self.provider,
                                       transcript: transcript.trimmingCharacters(in: .whitespacesAndNewlines))
    }

    private func validateOwner(_ owner: String) throws {
        guard Validation.safeID(owner) else { throw Abort(.unauthorized, reason: "A valid owner is required.") }
    }

    private func uncertainReplay() -> Abort {
        Abort(.conflict, reason: "A durable call intent already exists, but its outcome is uncertain. This request will not dial again; review ElevenLabs before any new call.")
    }
    ...
    private func ensureDirectory() throws {
        ...
        } catch { throw Abort(.serviceUnavailable, reason: "Call receipt storage is unavailable or already in use by another server instance. No new call was placed.") }
    }
    ...
    private func read(owner: String, requestID: String) throws -> VoiceCallReceipt? {
        ...
        do {
            ...
            guard receipt.version == 1, receipt.owner == owner, receipt.request.requestID == requestID,
                  ["intent", "uncertain", "initiated", "in-progress", "processing", "done", "failed"].contains(receipt.status),
                  receipt.conversationID.map(Self.safeConversationID) ?? true else { throw CocoaError(.fileReadCorruptFile) }
            return receipt
        } catch { throw Abort(.serviceUnavailable, reason: "The existing call receipt cannot be verified. No new call will be placed; inspect receipt storage.") }
    }
    ...
    private func write(_ receipt: VoiceCallReceipt) throws {
        ...
        do {
            try FileManager.default.createDirectory(at: ownerDirectory, withIntermediateDirectories: true, attributes: [.posixPermissions: 0o700])
        ...
        } catch { throw Abort(.serviceUnavailable, reason: "The durable call receipt could not be saved. Do not retry with a new request ID until the provider outcome is checked.") }
    }
}
...
private final class VoiceReceiptDirectoryLock: @unchecked Sendable {
    ...
    init(directory: URL) throws {
        ...
        guard descriptor >= 0 else { throw CocoaError(.fileWriteUnknown) }
        guard flock(descriptor, LOCK_EX | LOCK_NB) == 0 else { close(descriptor); throw CocoaError(.fileWriteUnknown) }
...
private struct ElevenOutboundResponse: Decodable { let success: Bool; let conversation_id: String? }

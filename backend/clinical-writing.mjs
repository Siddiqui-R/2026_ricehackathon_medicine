// Purpose: Keep all generated clinical wording precise, source-bound and readable by the patient.
// Inputs: Shared server instructions and generated prose fields selected by an operation's validator.
// Outputs: One writing policy and a narrow rejection check for prohibited punctuation and boilerplate.
// Side effects: None; never edits source documents, excerpts, transcripts or generated text.

// MARK: - Shared policy applies before operation-specific schemas and length limits
export const CLINICAL_WRITING_POLICY = `
...
// MARK: - Deliberately narrow style checks do not reject legitimate medical vocabulary
// Match sentence-opening boilerplate, not isolated words such as "foster", "overall" or "significant".
const boilerplate =
  /(?:^|[.!?\r\n])\s*(?:as (?:an? ai(?: language model| assistant)?|a language model)(?=\s*[,;:.!?]|\s+(?:i|we|my|our)\b)|(?:i )?hope this helps\b|i hope this finds you well\b|(?:let['’]s|let us) delve\b|here(?:['’]s| is) (?:a |an |the )?(?:(?:comprehensive|detailed|concise|helpful|brief) )?(?:summary|overview)\b|it(?:['’]s| is) (?:important to note|worth noting) that\b|in conclusion\b|to sum up\b|rest assured\b|your health journey\b)/iu;
const sourceFraming =
  /(?:\b(?:the |this )?(?:transcript|recording|conversation|discussion|report|document|note)s?\s+(?:(?:discuss|mention|cover|describe|address)(?:es|s)?|talks? about|focus(?:es)? on|(?:is|are) about)\b|\b(?:discussed|mentioned|covered|described|addressed) in (?:the |this )?(?:transcript|recording|conversation|discussion|report|document|note)s?\b)/iu;

export class ClinicalWritingError extends Error {
  constructor(violation, field) {
    super("Generated clinical wording did not meet the writing criteria.");
    this.name = "ClinicalWritingError";
    this.violation = violation;
    this.field = field;
  }
}

export function validateGeneratedClinicalText(value, label = "Generated text") {
  if (typeof value !== "string")
    throw new ClinicalWritingError("invalid-text", label);
  if (/[\u2013\u2014]/u.test(value))
    throw new ClinicalWritingError("unsupported-dash", label);
  if (boilerplate.test(value))
    throw new ClinicalWritingError("boilerplate", label);
  if (sourceFraming.test(value))
    throw new ClinicalWritingError("source-framing", label);
}
